  ...
  return (
    <>
      <section className={clsx (
        `${maruburi.className} font-semibold relative h-1/2 w-full border-t border-text-700 pt-8 pl-8 overflow-y-scroll transition-all duration-300`,
        goUp ? '-top-8' : 'top-1/2'
      )}>
        {tempNotes.map((note) => (
          <div
            key={note.id}
            className={clsx (
              "text-nowrap h-12 w-full transition-[opacity] duration-300",
              hovered && hovered !== note.id && "opacity-20"
            )}
